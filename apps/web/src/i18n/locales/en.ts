...
export const en: Dict = {
  ...
  'settings.welcomeTitle': 'Set up FBR-Design Studio',
  ...
  'settings.agentInstall.pathHint':
    'If you installed a CLI with npm or Homebrew and it still shows as not installed, ensure the tool\'s bin directory is on the PATH the FBR-Design Studio daemon inherits (Terminal vs GUI apps can differ on macOS). See QUICKSTART.md (section "Local agent CLI and PATH").',
  'settings.agentInstall.stepOpenLinks': 'Open Install or Docs for your preferred agent.',
  'settings.agentInstall.stepAuth':
    'Authenticate with the vendor CLI (sign in or add API credentials) before returning to FBR-Design Studio.',
  ...
  'settings.mcpServerHint': 'Expose FBR-Design Studio as an MCP server for your coding agent.',
  ...
  'settings.mcpHint':
    'Lets a coding agent in another repo (Claude Code, Cursor, VS Code, Antigravity, Zed, Windsurf) read your FBR-Design Studio projects. Use it to pull a design into your app without exporting a zip first.',
  'settings.mcpDaemonError':
    "Couldn't reach the local daemon to resolve install paths ({error}). Make sure FBR-Design Studio is running, then reopen this panel.",
  ...
  'settings.mcpCapabilityDefault':
    'Default to the project and file you have open in FBR-Design Studio, so you can say "build this in my app" without re-stating which design.',
  'settings.mcpRunningNote':
    'FBR-Design Studio must be running for MCP tool calls to succeed. If you started your coding agent before opening FBR-Design Studio, restart the agent so it can reach the live daemon.',
  ...
  'preview.errorBody': 'The example HTML failed to fetch. Make sure FBR-Design Studio is running and try again.',
  'preview.retry': 'Retry',
  ...
  'pet.tabBuiltInHint': 'Curated companions bundled with FBR-Design Studio — pick one and adopt.',
  ...
  'pet.codexBundledTitle': 'Ships with FBR-Design Studio — no download needed.',
