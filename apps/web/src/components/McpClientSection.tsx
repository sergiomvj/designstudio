...
//
// FBR-Design Studio connects to the configured servers as a CLIENT and surfaces
...
// so the most useful categories for FBR-Design Studio (visual generation, then
...
export const McpClientSection = forwardRef<McpClientSectionHandle, Props>(
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (cancelled) return;
      if (!data) {
        setError('Could not reach the local daemon. Make sure FBR-Design Studio is running, then reopen this panel.');
        setLoaded(true);
        return;
      }
        ...
        <>
          ...
          {isHttpLike && row._isNew ? (
            <div className="mcp-oauth-hint hint">
              Save first, then click <strong>Connect</strong> to grant FBR-Design Studio
        ...
        {connected ? (
          ...
          <>
            ...
            <span>
              ...
              <span className="hint">
                Click Connect to grant FBR-Design Studio access via the provider's OAuth flow.
              </span>
            </span>
          </>
        )}
