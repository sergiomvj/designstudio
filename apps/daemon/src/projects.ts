...
function buildDesignManifest(entries, projectLabel) {
  ...
  return JSON.stringify({
    ...
    title: projectLabel || 'FBR-Design Studio project',
    entryFile,
    sourceFiles: {
      all: files,
      html: htmlFiles,
...
function buildDesignHandoff(entries, projectLabel) {
  ...
  return `# ${projectLabel || 'FBR-Design Studio project'} implementation handoff
...
- Keep generated product UI free of FBR-Design Studio chrome, preview labels, or design-process annotations.
...
- Do not keep prototype-only annotations, frame labels, or FBR-Design Studio chrome in the production UI.
