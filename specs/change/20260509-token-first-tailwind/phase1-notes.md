...
### Step 2: FBR-Design Studio Tailwind tokens
...
- `scripts/guard.ts` - added default Tailwind palette utility rejection for classes such as `text-red-500`, `bg-white`, `border-zinc-200`, `from-orange-500`, and related color utility namespaces so app UI uses FBR-Design Studio token utilities.
...
- Style guard allowlist and promotion policy:
  ...
  | Repeated arbitrary app UI colors | Any unregistered hardcoded color after the second real app UI use | Promote to a named FBR-Design Studio token before migration. One-off brand/user-content/illustration data keeps a reasoned allowlist entry. |
...
- `pnpm --filter @open-design/web build` - passed after adding the FBR-Design Studio `@theme` token aliases and source variables.
