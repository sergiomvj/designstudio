...
- Local packaging orchestration for packaged FBR-Design Studio artifacts.
...
- Release artifacts keep canonical app identity (`FBR-Design Studio.app` on mac, `FBR-Design Studio.exe` inside the Windows installer); local tools-pack installs may use namespace-scoped install paths only as a developer multi-instance validation convention.
